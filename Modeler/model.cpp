...
SpotLight::SpotLight(const char* name, GLuint lightNumber,
	...
	cutoff("Cutoff", 0, 90, coff, .01f),
	angular("Angular Falloff", 0, 10, alar, .01f)
{
	properties.add(&attenA).add(&attenB).add(&attenC)
		.add(&spotX).add(&spotY).add(&spotZ)
		.add(&cutoff).add(&angular);
}
...
void SpotLight::draw() {
	...
	GLfloat light_direction[] = { (GLfloat)spotX.getValue(), (GLfloat)spotY.getValue(), (GLfloat)spotZ.getValue() };
	glLightfv(lightNumber, GL_SPOT_DIRECTION, light_direction);
	glLightf(lightNumber, GL_SPOT_EXPONENT, angular.getValue());
	Light::draw(false);
}
